  ...
  return (
    <Container>
      ...
      {consult.transcript && Object.keys(consult.transcript).length > 0
        ? renderTranscript2(consult)
        : renderLoading('Processing Consult')}
    </Container>
  );
}
    ...
    <Row>
      ...
      <Col md='2'>
        <h3>
          <Badge
            variant='info'
            style={{ color: textColor, backgroundColor: 'rgba(0,0,0,1)' }}
          >
...
const Consult = (props) => {
  ...
  return consult ? renderConsult(consult) : renderLoading('Loading Consult');
};
